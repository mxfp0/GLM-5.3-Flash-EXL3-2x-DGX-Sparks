...
def _pad_then_narrow(
    tensor: torch.Tensor, dim: int, start: int, length: int
) -> torch.Tensor:
    """Pad a checkpoint tensor when a TP pad made shard_end past the stored dim.

    GLM-5.3 TP=3 pads moe_intermediate_size 2048→2112. Rank-2 row-parallel
    shards start at 1408 with length 704 (need 2112; disk has 2048).
    """
    if tensor.dim() == 0:
        return tensor
    need = start + length
...
class _ColumnvLLMParameter(BasevLLMParameter):
    ...
    def load_column_parallel_weight(self, loaded_weight: torch.Tensor):
        ...
        loaded_weight = _pad_then_narrow(
