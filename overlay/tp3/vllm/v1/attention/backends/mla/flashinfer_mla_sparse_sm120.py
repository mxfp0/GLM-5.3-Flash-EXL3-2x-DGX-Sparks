...
class FlashInferMLASparseSM120Impl(MLAAttentionImpl[FlashInferMLASparseMetadata]):
    ...
    def forward_mqa(
        ...
        # glm53-tp3: the upstream overlay padded only decode-sized calls
        # (<= 64 tokens). sparse_mla_prefill_dispatch accepts the SAME head
        # table as decode -- dispatch_v32 takes num_heads 8 or 16 below HPB and
        # multiples of HPB above, so local 22 at TP=3 is rejected there too:
        #   Unsupported sparse-MLA prefill configuration: model=GLM_NSA
        #   num_heads=22 topk=2048 page_block_size=64
        # which killed the engine on the first real prompt. Pad for every call
        # size, not just decode. The reasoning is unchanged: softmax is
        # per-head, so the extra heads carry zero Q, never mix with the real
        # heads, and are sliced off below.
        kernel_heads = _decode_kernel_heads(self.num_heads)
        q_kernel = q
        if kernel_heads != self.num_heads:
            # [T, H, D] → pad H on the right.
